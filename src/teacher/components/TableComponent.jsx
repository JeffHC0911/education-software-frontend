...
export const TableComponent = ({ courseId }) => {
  ...
  return (
    <div>
      ...
      >
        ...
        <Table.Head>
          ...
          {filterAssesments.map((column) => (
            <Table.HeadCell className="min-w-[152px]" key={column._id}>
              {column.name} - {column.weighted} %
            </Table.HeadCell>
          ))}
              ...
              <Table.Row key={student.id} className="bg-white">
                ...
                {course.assessments.map((assessment, index) => (
                  <Table.Cell key={index}>
                    {assessment.grades[index] && assessment.grades[index].value}
                  </Table.Cell>
                ))}
                <Table.Cell>
                  <div className="flex items-center gap-3">
      ...
      {/* Renderiza el componente AddGradesComponent si hay un estudiante seleccionado */}
      {selectedStudentId && (
